	...
	*/
	...
	foreach($res as $campo => $valor){
		...
		if($campo == "CANDIDATO"){
			...
			if(filesize($ruta_foto."FOTO_$folio-00$res[candidato].jpg")>0){
			$pdf->Cell(20,30,$pdf->Image($ruta_foto."FOTO_$folio-00$res[candidato].jpg",$valx+0.5,$valy+6,20,0),0,0);
			}else{
			#$pdf->Cell(20,30,$pdf->Image("./src/pixel.jpg",$valx+0.5,$valy+6,20,0),0,0);
			$pdf->Cell(20,30,$pdf->Image("./src/sinfoto.jpg",$valx+0.5,$valy+6,20,0),0,0);
			}
		}else{
			$pdf->setX($valx+20);
			$pdf->setFont('Arial','',8);
...
#$nombre_arch = "Testimoniales_Prueba.pdf";
//obtengo los folios de las cedulas a generar
$qry = "select folio_def from lis_defunciones_def where id_est=2 and cve_ent_def='$entidad' and cve_mun_def='$mun' order by cve_mun_def, colonia_def, folio_def;";
#$qry = "select folio_def from lis_defunciones_def where id_est=2 and cve_ent_def='$entidad' order by folio_def limit 1000;";
