	...
	*/
	...
	foreach($res as $campo => $valor){
		...
		if($campo == "CANDIDATO"){
			...
			$this->Cell(20,30,$this->Image($ruta_foto."FOTO_$folio-00$res[candidato].jpg",$valx+0.5,$valy+6,20,0),0,0);
			}else{
			$this->Cell(20,30,$this->Image("./src/sinfoto.jpg",$valx+0.5,$valy+6,20),0,0);
			}
		}else{
			$this->setX($valx+20);
			$this->setFont('Arial','',8);
...
$entidad = $argv[1];
$mun = $argv[2];
$nombre_arch = "Testimoniales_".$entidad."_".$mun."_puebas.pdf";
$qry = "select folio_def from lis_defunciones_def where id_est=2 and cve_ent_def='$entidad' order by folio_def limit 5;";
