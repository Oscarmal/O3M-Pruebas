...
class plantilla{
        function plantilla($template_file){
                $this->tpl_file = 'tpl/' . $template_file . '.tpl';
        }
        ...
        function muestra(){
                if (!($this->fd = @fopen($this->tpl_file, 'r'))) {
                        #sostenedor_error('error al abrir la plantilla ' . $this->tpl_file); // Checar función para mostar error al no encontrar la plantilla
						echo "Error al abrir el archivo " . substr($this->tpl_file,4,20);
                } else{
